...
def create_namespace(name, description):
    """Create a namespace"""
    response = requests.post(f"{BASE_URL}/namespaces/", json={"name": name, "description": description})
...
def create_schema(name, structure):
    """Create a schema"""
    response = requests.post(f"{BASE_URL}/schemas/", json={"name": name, "structure": structure})
    if response.status_code == 200:
...
def create_config(namespace_id, schema_id, key, value):
    ...
    response = requests.post(
        f"{BASE_URL}/configs/", json={"namespace_id": namespace_id, "schema_id": schema_id, "key": key, "value": value}
    )
    if response.status_code == 200:
...
def seed_simple_namespace(ns_id, ns_name):
    ...
    schema_id = create_schema(
        f"{ns_name.upper()}_Config", {"type": "object", "additionalProperties": {"type": "string"}}
    )
    ...
    if schema_id:
        ...
        create_config(
            ...
            schema_id,
            "feature_flags",
            {"enable_new_ui": "true", "enable_beta_features": "false", "maintenance_mode": "false"},
        )
        ...
        create_config(
            ...
            schema_id,
            f"{ns_name}_settings",
            {"max_connections": "100", "timeout_seconds": "30", "retry_attempts": "3", "log_level": "INFO"},
        )
