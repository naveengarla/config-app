...
def create_schema(name, description, structure):
    resp = requests.post(f"{BASE_URL}/schemas/", json={"name": name, "description": description, "structure": structure})
...
def create_config(ns_id, schema_id, key, value):
    requests.post(
        f"{BASE_URL}/configs/", json={"namespace_id": ns_id, "schema_id": schema_id, "key": key, "value": value}
    )
...
def seed():
    ...
    list_schema_id = create_schema("IPWhitelist", "List of allowed IP addresses", {"type": "array", "items": {"type": "string"}})
    if list_schema_id:
        create_config(net_ns, list_schema_id, "admin_ips", ["127.0.0.1", "192.168.1.1", "10.0.0.5"])

    # 2. TABLE Example: Routing Rules
    table_schema_id = create_schema(
        "RoutingRules",
        "Traffic routing rules and weights",
        {
            "type": "array",
            "items": {
                "type": "object",
    ...
    hierarchy_schema_id = create_schema(
        "FeatureRollout",
        "Complex feature flag strategy",
        {
            "type": "object",
            "properties": {
    ...
    if hierarchy_schema_id:
        create_config(
            ...
            {
                "enabled": True,
                "strategy": {"type": "gradual", "percentage": 25, "users": ["user_123", "user_456"]},
