...
def read_namespaces(skip: int = 0, limit: int = 100, db: Session = Depends(get_db)):
    namespaces = (
        db.query(models.Namespace).filter(models.Namespace.deleted_at.is_(None)).offset(skip).limit(limit).all()
    )
    return namespaces
...
def read_namespace(namespace_id: int, db: Session = Depends(get_db)):
    namespace = (
        ...
        .filter(models.Namespace.id == namespace_id, models.Namespace.deleted_at.is_(None))
        .first()
    )
