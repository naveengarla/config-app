...
def get_reference_data(
    ...
    config = (
        ...
        .filter(models.ConfigEntry.namespace_id == ns.id, models.ConfigEntry.key == key)
        .first()
    )

    if not config:
        raise HTTPException(status_code=404, detail=f"Config '{key}' not found in namespace '{namespace}'")
...
def lookup_item_in_array(
    ...
    if not isinstance(config.value, list):
        raise HTTPException(
            status_code=400, detail="Config is not an array type. Use GET /reference/{namespace}/{key} instead."
        )
    ...
    raise HTTPException(status_code=404, detail=f"Item with {id_field}='{id}' not found")


@router.get("/{namespace}/{key}/search")
def search_in_config(
    namespace: str, key: str, q: str = Query(..., description="Search query"), db: Session = Depends(get_db)
):
    """
    ...
    if q_lower in config_str:
        ...
        if isinstance(config.value, list):
            ...
            return {"results": [config.value], "count": 1, "query": q}
