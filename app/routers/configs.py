...
def validate_config_value(value, schema_structure):
    try:
        ...
        raise HTTPException(status_code=400, detail=f"Config validation failed: {e.message}") from e
...
def create_config(config: schemas.ConfigEntryCreate, db: Session = Depends(get_db)):
    ...
    existing_config = (
        db.query(models.ConfigEntry)
        .filter(models.ConfigEntry.namespace_id == config.namespace_id, models.ConfigEntry.key == config.key)
        .first()
    )
    if existing_config:
    ...
    new_config = models.ConfigEntry(
        namespace_id=config.namespace_id, schema_id=config.schema_id, key=config.key, value=config.value
    )
    db.add(new_config)
    db.commit()
    ...
    history = models.ConfigHistory(config_id=new_config.id, value=new_config.value, version=new_config.version)
    db.add(history)
    db.commit()
...
def read_config(config_id: int, db: Session = Depends(get_db)):
    config = (
        db.query(models.ConfigEntry)
        .filter(models.ConfigEntry.id == config_id, models.ConfigEntry.deleted_at.is_(None))
        .first()
    )
    if config is None:
...
def update_config(config_id: int, config_update: schemas.ConfigEntryUpdate, db: Session = Depends(get_db)):
    ...
    # Create history entry
    history = models.ConfigHistory(config_id=db_config.id, value=db_config.value, version=db_config.version)
    db.add(history)
